...
class Evaluator():
    ...
    def print_info(self):
        # f = open('../test.txt', 'a')
        # print('Prompt: {}, Attribute: {}'.format(self.test_prompt_id, self.attribute_name), file=f)
        ...
        #     self.best_dev[0], self.best_dev[1], self.best_dev[2], self.best_dev[3]), file=f)
        ...
        #     self.best_test[0], self.best_test[1], self.best_test[2], self.best_test[3]), file=f)
        #
        # print(
        #     '--------------------------------------------------------------------------------------------------------------------------', file=f)
        # f.close()
    ...
    def print_final_info(self):
        ...
        #     print('Prompt: {}, Attribute: {}'.format(self.test_prompt_id, self.attribute_name), file=file0)
        #     print('Best @ Epoch %i:' % self.best_dev_epoch, file=file0)
        #     print('  [DEV]  QWK: %.3f,  PRS: %.3f, SPR: %.3f, RMSE: %.3f' % (
        #     self.best_dev[0], self.best_dev[1], self.best_dev[2], self.best_dev[3]), file=file0)
        #     print('  [TEST] QWK: %.3f,  PRS: %.3f, SPR: %.3f, RMSE: %.3f' % (
        #     self.best_test[0], self.best_test[1], self.best_test[2], self.best_test[3]), file=file0)
        losshistory = LossHistory('logs_convnext/')
        print(
            '--------------------------------------------------------------------------------------------------------------------------')
        # print('Missed @ Epoch %i:' % self.best_test_missed_epoch)
        # print('  [TEST] QWK: %.3f' % self.best_test_missed)
        ...
        print('  [TEST] QWK: %.3f,  PRS: %.3f, SPR: %.3f, RMSE: %.3f' % (
        self.best_test[0], self.best_test[1], self.best_test[2], self.best_test[3]))
        losshistory.append_loss(self.test_prompt_id, self.attribute_name, self.best_dev_epoch, self.best_dev[0],
                                self.best_test[0])
